...
const YAHOO_API_KEY =
  process.env.YAHOO_API_KEY ||
  "Yahoo AppID: dj00aiZpPWc2Vk1NOFVsWmx5SCZzPWNvbnN1bWVyc2VjcmV0Jng9MGE-";
...
if (
  ...
  console.warn(
    "⚠️  Warning: Using default Yahoo API key. Please set YAHOO_API_KEY environment variable."
  );
}
...
app.post("/api/furigana", async (req, res) => {
  ...
  -d '${JSON.stringify(requestBody)}'`;
    ...
    const yahooResponse = await axios.post(YAHOO_API_URL, requestBody, {
      headers: {
    ...
    console.log(
      "📥 Yahoo API Response Data:",
      JSON.stringify(yahooResponse.data, null, 2)
    );
...
app.listen(PORT, () => {
  ...
  if (YAHOO_API_KEY) {
    ...
    console.log(
      `🔑 Yahoo API Key Preview: ${YAHOO_API_KEY.substring(0, 20)}...`
    );
  }
});
